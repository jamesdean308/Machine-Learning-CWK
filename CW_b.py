...
from sklearn.metrics import confusion_matrix # used for outputting the confusion matrix
...
def confusion_heat_map(matrix):
    plt.figure()
    ax = sns.heatmap(matrix, fmt=".1f")
    plt.show()
...
def predict_and_visualize_results(model, input_X, output_Y):
    ...
    # heat map
    confusion_heat_map(confusion_matrix(test_Y, predicted_classes))

    print("\n\nConfusion matrix:\n",confusion_matrix(test_Y, predicted_classes), "\n\nType:", type(confusion_matrix(test_Y, predicted_classes)))
...
if True:
    ...
    #------------------------------------Architecture---------------------------------
    model.add(Conv2D(8, kernel_size=(3, 3),activation='relu',input_shape=(28,28,1),padding='same')) # EDIT changed dimension to 8

    model.add(Dropout(0.1))# EDIT added drop out layer
    model.add(Conv2D(32, kernel_size=(3, 3),activation='relu',padding='same'))# EDIT added second Conv2D layer, dimensions 32
    model.add(MaxPooling2D((2, 2),padding='same'))

    model.add(Conv2D(32, kernel_size=(3, 3),activation='relu',padding='same'))# EDIT
    model.add(Conv2D(32, kernel_size=(3, 3),activation='relu',padding='same'))# EDIT

    model.add(Dropout(0.1))
    model.add(MaxPooling2D((2, 2),padding='same')) # EDIT
